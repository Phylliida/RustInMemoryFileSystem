...
class FS{
    ...
    public mounts : Array<FSMountInfo>;

    public constructor(storage: FileStorageInterface, qidcounter : QIDCounter) {
        /** @type {Array.<!Inode>} */
        this.inodes = [];
        this.events = [];
...
/** @constructor */
class INode {
    public direntries : Map<string, number>;
    ...
    public mode : number;
    public qid : QID;
    public caps : Nullable<Uint8Array>;
    public nlinks : number;
    public sha256sum : string;

    public locks : Array<FSLockRegion>;

    public mount_id : number;
    public foreign_id : number;

    public constructor(quidnumber : number) {
    ...
    public get_state() : Array<string | number | Array<FSLockRegion> | null | Array<number>> {
        const state = [];
        state[0] = this.mode;
    ...
    public set_state(state : Array<string | number | Array<FSLockRegion> | Array<number> | Array<Array<string | number>> | null>) : void {
        this.mode = state[0] as number;
        ...
        {
            [this.minor, this.major] = state[1] as Array<number>;
        }
        else
        {
            // Nothing
        ...
        this.status = state[3] as number;
        this.size = state[4] as number;
        this.uid = state[5] as number;
        this.gid = state[6] as number;
        this.fid = state[7] as number;
        this.ctime = state[8] as number;
        this.atime = state[9] as number;
        this.mtime = state[10] as number;
        this.qid.type = (this.mode & S_IFMT) >> 8;
        this.qid.version = state[11] as number;
        this.qid.path = state[12] as number;
        this.nlinks = state[13] as number;
...
{
    public fs : FS;
    public backtrack : Map<number, number>;

    public constructor(filesystem : FS) {
        /** @type {FS}*/
        this.fs = filesystem;
...
class FSLockRegion {
    ...
    public set_state(state : Array<any>) : void {
        this.type = state[0] as number;
        this.start = state[1] as number;
        this.length = (state[2] === 0 ? Infinity : state[2]) as number;
        this.proc_id = state[3] as number;
